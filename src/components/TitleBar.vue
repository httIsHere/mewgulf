<template>
  <div class="view-component view-component__title-bar">
    <div class="view-component__inner">
      <span class="title-bar__logo gradient-text">MewGulf</span>
      <div class="title-bar__tabs">
        <a :href="`#${item.id}`" :class="['title-bar__tabs-item', cur_tab == item.id ? 'title-bar__tabs-item-active' : '']" v-for="(item) in tabs" :key="item.id" @click="triggerTab(item)">{{item.name}}</a>
      </div>
    </div>
  </div>
</template>
        ...
        .title-bar__tabs{
            &-item {
                ...
                &-active {
                    ...
                    padding: 2px 20px;
                    border-radius: 50px;
                    box-shadow: 0 2px 10px rgba(241, 196, 15, .5);
                }
            }
        }
    }
